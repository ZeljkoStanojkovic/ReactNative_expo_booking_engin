...
class HotelOverviewC extends PureComponent {
    ...
    render() {
        ...
        if(hotel.policies) {
            ...
            let groupNames = rawPolicies.map(o=>o.type).filter(function(item, pos) {
                return a.indexOf(item) == pos;
            })
            policies = groupNames.map((type)=>{
                return {
                    title:type,
                    descriptions:rawPolicies.filter((o)=>o.type==type)
                }
            })
        }
        ...
        return (
            <View style={styles.container}>
                <ScrollView>
                    ...
                    />
                    {policies.length > 0 && <Policies policies={policies}></Policies>}
                </ScrollView>
