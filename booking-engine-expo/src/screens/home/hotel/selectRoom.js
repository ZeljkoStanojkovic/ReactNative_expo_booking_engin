...
const Room = ({ adults, children, infants, onPress, index, ageRange, onAgeChanged }) => (
    <View>
        <View style={styles.roomHeaderContainer}>
            <Text style={styles.headerText}>Room {index + 1}</Text>
        ...
        {children && children.map((child, index) => (
            ...
            <ModalDropdown
                ...
                renderRow={(option, index, isSelected) => <View style={styles.childItemContainer}>
                    <Text style={[styles.childAge, { color: isSelected ? Color.orange : Color.darkText }]}>{option==0?'<1':option}</Text>
                </View>}
            ...
            >
                <View style={styles.childContainer}>
                    ...
                    <Text style={styles.childAge}>{child==0?'<1':child}</Text>
...
class SelectRoomC extends PureComponent {
    constructor(props) {
        ...
        let childMinAge = this.paxConfigInfo.child.minAge
        let childMaxAge = this.paxConfigInfo.child.maxAge
        this.maxTotalPax = Global.environment.availableBusinesses.hotel.maxTotalPax
    ...
    addRoom() {
        if (this.maxTotalPax > 0) {
            if (this.state.rooms.length == this.maxTotalPax) {
                return
            }
        }
